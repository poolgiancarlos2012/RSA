<?php
    ...
    if($ope=='ckbx'){
        ...
        $sqldetdoc="  SELECT TRUNCATE(cant, 2) AS cant,codpro,um,prod,precio AS precio,dscto AS dscto,subtot AS subtot FROM sfw.detalle_documento WHERE iddocumento=$codigo AND ctd='$td' ORDER BY numdoc,item,prod;";
        ...
        ob_start();
        include(dirname(__FILE__).'/res/HTMLReporteFactura.php');
        ...
        $sqldetdoc="  SELECT TRUNCATE(cant, 2) AS cant,codpro,um,prod,precio AS precio,dscto AS dscto,subtot AS subtot FROM sfw.detalle_documento WHERE codage='$xcodage' AND ctd='$xctd' AND numser='$xnumser' AND numdoc='$xnumdoc' ORDER BY numdoc,item,prod;";
        ...
        try
        {
            ...
            $html2pdf->pdf->SetTitle('Reporte Factura');
            $html2pdf->pdf->SetDisplayMode('fullpage');
            $html2pdf->writeHTML($content, isset($_GET['vuehtml']));
