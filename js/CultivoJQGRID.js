var CultivoJQGRID={
	Consultar_Cultivo: function(){
		var jqrid="table_list_cultivo";
		var pager="pager_table_list_cultivo";
	    $("#"+jqrid).jqGrid({
	    	url:'../controller/ControllerAlfa.php?command=cultivo&action=ListCultivo',
			datatype:'json',
			gridview:true,
			width : 400,
			height:350,
			...
			forceFit:true,
			hidegrid: false,
			colNames:['ID','CULTIVO','ESTADO'],
			colModel:[
				{name:'idcultivo',index:'idcultivo',align:'center',width:60,hidden:true},
                {name:'cultivo',index:'cultivo',align:'left',width:200,editable:true},
	    ...
	    $("#"+jqrid).jqGrid('navGrid','#'+pager,
		    {
		    	edit:true,edittitle:"EDITAR CULTIVO", editicon: 'ui-icon-pencil',
                add:true,addtitle:"AGREGAR NUEVA CULTIVO", addicon:'ui-icon-plus',
                del:true,deltitle:"ELIMINAR CULTIVO",delicon:'ui-icon-trash',
		    	view:false,
		    	search:false,
		    	searchtitle:"Search",
		    	refresh: false
		    ...
		    {
                url:'../controller/ControllerAlfa.php?command=cultivo&action=Mant_Cultivo',
                mtype: 'GET',
                editCaption:"EDITAR CULTIVO",
                edittext:"Edit",
                bSubmit: "EDITAR",
                bCancel: "CANCELAR",                
                closeOnEscape:true, 
                ...
                beforeSubmit: function(postdata, formid){
                        ...
                        }else if(!alphanumeric.test(cultivo)){
                ...
                afterComplete:function(){
                	var cultivo=encodeURIComponent($("#cultivo").val());
                    $("#"+jqrid).jqGrid('setGridParam',{
                        datatype : 'json',
                        url:'../controller/ControllerAlfa.php?command=cultivo&action=ListCultivo&idcultivo=&cultivo='+cultivo
                    }).trigger("reloadGrid");
            ...
            },
            {
                ...
                beforeSubmit: function(postdata, formid){         		
                        ...
                        var alphanumeric=/^([a-z]|\d|\s|-|\.|_|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+$/i;
                        if(cultivo==''){
                            return [false,'POR FAVOR, INGRESE CULTIVO']; 
                        }else if(!alphanumeric.test(cultivo)){
                            return [false,'POR FAVOR, INGRESE LETRAS O NUMEROS'];
                        }else if($("#verifydup").val()>0){
                            return [false,'POR FAVOR, ESTE DATO YA EXISTE'];
            ...
            },
            {
                ...
                onclickSubmit: function(params, postdata) {//BOTON A GRABAR
                    ...
                    cultivo = encodeURIComponent($("#"+jqrid).jqGrid ('getCell', selRowId, 'cultivo'));
	    ...
	    function beforeShowAdd(formId) {
            ...
            setTimeout(function() {
		        document.getElementById('cultivo').className += ' textbox';
                $('#cultivo').attr('autocomplete','off');
                document.getElementById('cultivo').style.width += '200px';
                $('#estado').unwrap();
